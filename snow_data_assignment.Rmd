...
```{r}
meteorological_reader<-function(file){
  name=str_split_fixed(file,'/',2)[,2]
  nameRight=str_split_fixed(name, '_',2)[,2]
  nameLeft=str_split_fixed(name, '_',2)[,1]
  df<-read_fwf(file)%>%
    mutate(site=nameLeft)
  
}

SASP_Forcing<-read_fwf("data/SBB_SASP_Forcing_Data.txt")
?read_fwf
